...
envsubst '$ADMIN_PLACEHOLDER $FRONTEND_SERVER_NAME $ADMIN_GATEWAY $API_PLACEHOLDER $UPLOADS_PLACEHOLDER $BACKEND_GATEWAY $FTP_FILES_PLACEHOLDER $SSL_CERTIFICATE $SSL_CERTIFICATE_KEY $SSL_CERTIFICATE_TRUSTED $ONEC_GATEWAY $API_ONEC_PLACEHOLDER' < /etc/nginx/conf.d/default.conf.template > /etc/nginx/conf.d/default.conf
